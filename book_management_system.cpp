//////////////////////////////////////////////////////////////////
// before running the code, make sure the sql server is started //
// using: 'brew services start mysql' in the terminal.          //
// Subsequently, to stop the server use:                        //
...
void main_menu() {
    ...
    while (!validInput) {
        ...
        if (cin.fail()) {
            ...
            // If so, the input contains characters other than the valid integer
            // Clear the input buffer to remove the invalid characters
            if (cin.peek() != '\n') {
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
                cout << "Invalid input. Please enter a number between 1 and 7." << endl;
            } else {
    ...
    switch (c) {
        ...
        default:
            // This case should never be reached with the validation in place
...
int main() {
    try {
        ...
        if (conn) {
            ...
            throw runtime_error("Error while connecting to database. Contact Tech Expert.");
        }
